  ...
  try {
    ...
    if (isAlreadyOverdue) {
      ...
      loadRow.innerHTML = `<td colspan="${colspan}" style="background:var(--bg3);border-left:3px solid var(--yellow);padding:14px 20px">
        ...
        ${aiSummaryPlaceholder(salesOrderId)}
      </td>`;
      btnEl.textContent = 'Hide';
      btnEl.disabled = false;
      loadAiDelaySummary(salesOrderId, {
        runId: result.run?.id,
        promise_week: `W${pWeek}/${pYear}`,
        optimized_date: optDate,
    ...
    loadAiDelaySummary(salesOrderId, {
      runId: result.run?.id,
      promise_week: `W${pWeek}/${pYear}`,
      optimized_date: orderResult?.optimized_date,
      delay_days: orderResult?.delay_days,
      penalty_cost: orderResult?.penalty_cost,
      already_overdue: false,
      capacity_issues: capIssues.map(c => ({
        name: c.restriction_name, code: c.restriction_code,
        capacity: c.capacity, required: c.required_capacity, over_capacity: c.over_capacity
      })),
      component_issues: compIssues.map(c => ({
        name: c.component_name, code: c.component_code,
        available: c.available, required: c.required, shortage: c.shortage
      }))
    });
  } catch(e) {
    loadRow.remove();
...
// ─── AI SUMMARY (SAP AI Core orchestration, via /api/ai/delay-summary) ────────
function aiSummaryPlaceholder(salesOrderId) {
  // AI summary disabled. Returning '' (not undefined) matters: this is
  // interpolated into a template literal, and undefined renders as "undefined".
  // With no #ai-sum-* element in the DOM, loadAiDelaySummary() exits at its
  // !host guard before issuing any request, so no AI Core call is made.
  return '';

  // return `<div id="ai-sum-${salesOrderId}" style="margin-top:14px;padding-top:12px;border-top:1px solid var(--border)">
  //   <div style="display:flex;align-items:center;gap:8px;font-size:11px;font-weight:600;text-transform:uppercase;letter-spacing:0.07em;color:var(--text3)">
  //     <span>AI Summary</span>
  //     <div class="spinner" style="width:11px;height:11px"></div>
  //   </div>
  //   <div style="margin-top:6px;font-size:12px;color:var(--text3)">Generating explanation…</div>
  // </div>`;
}

async function loadAiDelaySummary(salesOrderId, payload) {
  const host = document.getElementById('ai-sum-' + salesOrderId);
  if (!host) return;

  const shell = (bodyHtml, footer = '') => `
    <div style="display:flex;align-items:center;justify-content:space-between;gap:8px">
      <div style="font-size:11px;font-weight:600;text-transform:uppercase;letter-spacing:0.07em;color:var(--text3)">AI Summary</div>
      ${footer}
    </div>
    <div style="margin-top:7px">${bodyHtml}</div>`;

  try {
    const { summary, model } = await api('POST', '/ai/delay-summary', { salesOrderId, ...payload });

    // The model is instructed to emit "Root cause:/Impact:/Recommended action:".
    // Bold those labels when present; otherwise render the paragraphs as-is.
    const body = summary.split(/\n+/).filter(Boolean).map(line => {
      const m = line.match(/^\s*(Root cause|Impact|Recommended action)\s*:\s*(.*)$/i);
      return m
    ...
    // Never destroy the deterministic tables above — just note the AI part failed.
    host.innerHTML = shell(
      `<div style="font-size:12px;color:var(--text3)">AI explanation unavailable — ${esc(e.message)}</div>`);
  }
}
...
function renderOptimizationResults(result) {
  ...
  const compTabHtml = `<div id="tab-comps" style="display:none">${comps.length===0?'<div class="card"><div class="card-body text-muted text-sm">No component data</div></div>':''}${compBarHtml}<div class="card"><div class="card-header"><div class="card-title">Weekly Component Status</div></div><div class="card-body" style="padding:0"><div class="table-wrap"><table><thead><tr><th>Component</th><th>Week</th><th>Available</th><th>Required</th><th>Utilization</th><th>Shortage</th><th>Shortage Cost</th><th>Status</th></tr></thead><tbody>${
    comps.map(c=>{const p=utilRatio(c.required,c.available);const col=c.is_critical?'var(--red)':p>=80?'var(--yellow)':'var(--green)';return `<tr><td><div style="font-weight:600;font-size:13px">${c.component_name}</div><div class="mono text-xs text-muted">${c.component_code}</div></td><td class="mono">W${c.week}/${c.year}</td><td class="mono text-green">${Math.round(c.available||0)}</td><td class="mono">${Math.round(c.required||0)}</td><td><div class="flex items-center gap-2"><div class="progress-bar" style="width:70px"><div class="progress-fill" style="width:${fmt.utilBar(p)}%;background:${col}"></div></div><span style="color:${col};font-size:11px;font-family:var(--mono)">${fmt.utilPct(p)}</span></div></td><td class="${c.is_critical?'text-red':'text-green'}">${Math.round(c.shortage||0)}</td><td class="${c.shortage_cost>0?'text-red':''}">${fmt.penalty(c.shortage_cost)}</td><td>${c.is_critical?'<span class="badge badge-critical">Shortage</span>':'<span class="badge badge-ok">OK</span>'}</td></tr>`;}).join('')
